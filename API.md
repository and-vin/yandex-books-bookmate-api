...
|---|---|---|
| GET | `/profile/library_cards?limit=50&offset=0` | `offset` и `limit` выше ~20 не влияют на ответ — всегда возвращаются те же ~20 самых свежих карточек. Для полного листинга использовать `page`/`per_page` (см. ниже), не `offset`/`limit`. Не использовать для проверки «уже ли книга в библиотеке» — надёжнее ответ `POST /profile/library_cards` |
| GET | `/profile/library_cards?page=N&per_page=M` | Рабочая пагинация для этого эндпоинта: `per_page` реально задаёт размер страницы, `page=2` отдаёт следующие M карточек без пересечения с `page=1`. Годится для полного листинга личной библиотеки |
