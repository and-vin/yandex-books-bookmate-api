...
|---|---|---|
| GET | `/profile/library_cards?page=N&per_page=M` | `per_page` задаёт размер страницы, `page=2` отдаёт следующие M карточек без пересечения с `page=1` — годится для полного листинга личной библиотеки. Не использовать для проверки «уже ли книга в библиотеке» — надёжнее ответ `POST /profile/library_cards` |
